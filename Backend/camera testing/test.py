...
// rtmp h264 camera output example no encoding
...
// record from rtmp stream to mp4 file
ffmpeg -i rtmp://localhost:1935/stream -c copy -f mp4 recording.mp4

// rtsp h264 camera output example no encoding
ffmpeg -f v4l2 -input_format h264 -framerate 30 -video_size 1920x1080 -i /dev/video0 \
-c copy -f rtsp -rtsp_transport tcp -muxdelay 0.1 rtsp://localhost:8554/stream
...
if __name__ == "__main__":
    ...
    host = os.getenv("PURR_CAMERA_BIND_HOST", "0.0.0.0")
    port = int(os.getenv("PURR_CAMERA_BIND_PORT", "8001"))
    uvicorn.run(app, host=host, port=port, reload=False, proxy_headers=True)
